/**
 * status feature unit tests
 * @module welcome/status
 * @file src/api/welcome/features/status/status.test.ts
 * 
 * @llm-rule WHEN: Testing status service endpoints with mocked AppKit dependencies
...
describe('Status Feature', () => {
  ...
  describe('GET /status', () => {
    it('should return default greeting from welcome/status', async () => {
  ...
  describe('GET /status/:name', () => {
    it('should return personalized greeting from welcome/status', async () => {
      const response = await request(app)
        .get('/status/Alice')
        .expect(200);
      ...
      expect(response.body).toMatchObject({
        success: true,
        data: {
          message: 'Hello from welcome/status, Alice!',
          app: 'welcome',
          feature: 'status',
          name: 'Alice',
          version: '1.0.0',
