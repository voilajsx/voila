# VoilaJSX - Framework Scripts Documentation
...
| **[doc](#doc-script)** | Documentation generation | `npm run doc` | Generate TypeDoc documentation |
...
- **voila-server.ts** (18KB) - Server lifecycle management
- **voila-routes.ts** (11KB) - Route analysis and validation  
- **voila-doc.ts** (5KB) - Documentation generation

### 🎯 Code Generation & Scaffolding
- **voila-generate.ts** (144KB) - Full-stack application and feature generator
- **voila-plan.ts** (46KB) - Agentic planning workflow with human oversight

### 🔧 DevOps & Deployment
- **voila-deploy.ts** (8KB) - Deployment automation with Fly.io integration
- **voila-git.ts** (19KB) - Git workflow automation
- **voila-validate.ts** (21KB) - Code quality and structure validation

### 🧪 Testing & Quality
- **voila-test.ts** (57KB) - Comprehensive testing orchestration
- **voila-prisma.ts** (13KB) - Database schema management

### 📖 Documentation & Help
- **voila-help.ts** (21KB) - Interactive help system
- **voila-context.ts** (41KB) - AI context and workflow management
...
**Purpose:** Generate TypeDoc documentation with browser integration.
...
npm run doc -- --open            # Generate and open in browser
npm run doc -- -o               # Generate and open (short form)
npm run doc open                 # Open existing documentation
npm run doc -- --help           # Show help options
```

### Features
- **TypeDoc Integration:** Generate comprehensive API documentation
- **Browser Support:** Automatic browser opening with cross-platform support
- **TSDoc Compliance:** Supports all TSDoc tags and custom Voila extensions
- **Module Organization:** Clean documentation structure by feature

### When to Use
- **Development:** Generate documentation for code review
- **Documentation:** Create API references for team use
- **Onboarding:** Help new developers understand codebase
- **Release:** Generate documentation for releases
...
**Purpose**: Comprehensive help system for all Voila commands
```bash
# Show all commands
npm run help
...
#### `voila-context.ts` - Workflow Management
**Purpose**: AI context management and workflow orchestration
```bash
# Framework context
npm run context voila:framework

# Comment standards context
...
```
**Capabilities**:
- Structured context delivery for AI assistants
- Workflow step tracking and management
- Progress persistence across sessions
- Integration with other Voila scripts

## Script Architecture Features

### Unified Design Patterns
- **TypeScript-first**: All scripts written in TypeScript with proper typing
- **Consistent CLI**: Standardized command-line interfaces across all scripts
- **Error Handling**: Comprehensive error management with debugging modes
- **Progress Tracking**: Visual progress indicators and status reporting

### Integration Points
- **Workflow Management**: Scripts communicate through shared workflow state
- **Configuration**: Centralized configuration management
- **Logging**: Consistent logging and debugging across all operations
- **Validation**: Built-in validation at every step

### Development Experience
- **Hot-reload Integration**: Server management works seamlessly with development
- **IDE Integration**: Scripts designed to work with VS Code and other IDEs
- **CI/CD Ready**: All scripts support continuous integration environments
...
npm run generate app:api myapp/newfeature

# 2. Validate structure
npm run validate myapp

# 3. Run feature tests
npm run test -- --feature newfeature

# 4. Update documentation
npm run doc

# 5. Git integration
npm run git commit "feat: add newfeature"
```
...
- **Always validate before deployment**: Use `npm run validate` before `npm run deploy`
- **Test incrementally**: Use feature-specific testing during development
- **Plan complex features**: Use `npm run plan` for multi-step development
- **Monitor server state**: Use `npm run server status` to check running processes

### Integration Tips
- **Combine scripts**: Many scripts work together (generate → validate → test → deploy)
- **Use help system**: `npm run help [command]` provides detailed usage information
- **Leverage workflows**: Context management maintains state across development sessions
- **Excel integration**: Test scripts generate Excel reports for team collaboration

### Troubleshooting
- **Debug mode**: Set `DEBUG=1` environment variable for detailed logging
- **Server conflicts**: Use `npm run server status` to identify port conflicts
- **Validation errors**: Run `npm run validate -- --verbose` for detailed error reporting
- **Test failures**: Use `npm run test -- --debug` for test debugging

---

**Quick Reference**: Run `npm run help` to see all available commands with examples and options.