        ...
        <nav className="hidden items-center gap-6 md:flex" aria-label="Primary">
          {nav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="text-sm font-medium text-slate-900/80 hover:text-slate-900"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="mx-auto flex max-w-6xl flex-col gap-1 px-4 py-2">
          {nav.map((item) => (
            <Link
              ...
              className="rounded px-2 py-2 text-sm font-medium text-slate-900/80 hover:bg-slate-100 hover:text-slate-900"
            >
              {item.label}
            </Link>
          ))}
