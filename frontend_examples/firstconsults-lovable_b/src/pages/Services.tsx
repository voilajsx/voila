    ...
    <div className="min-h-screen">
      ...
      <section className="pt-32 pb-20 px-4 sm:px-6 lg:px-8 bg-warm-gradient relative overflow-hidden">
        <div className="absolute inset-0 opacity-40">
          <div className="absolute top-10 left-20 w-36 h-36 bg-accent-gradient rounded-full blur-3xl animate-float"></div>
          <div className="absolute bottom-20 right-20 w-52 h-52 bg-hero-gradient rounded-full blur-3xl animate-float" style={{animationDelay: '1.5s'}}></div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((service, index) => (
              <Card key={index} className="bg-card-gradient shadow-soft hover:shadow-glow transition-all duration-500 h-full hover:scale-105 transform group animate-slide-up" style={{animationDelay: `${index * 0.1}s`}}>
                <CardHeader>
                  <div className="w-16 h-16 bg-hero-gradient rounded-full flex items-center justify-center mb-4 group-hover:animate-glow-pulse">
      ...
      <section className="py-20 bg-muted/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold mb-4 animate-slide-up">Consulting Packages</h2>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {packages.map((pkg, index) => (
              <Card 
                key={index} 
                className={`relative bg-card-gradient shadow-soft hover:shadow-glow transition-all duration-500 hover:scale-105 transform animate-slide-up group ${
                  pkg.popular ? 'ring-2 ring-primary shadow-glow border-primary/30' : ''
                }`}
                style={{animationDelay: `${index * 0.2}s`}}
              >
                {pkg.popular && (
                  <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
                    <span className="bg-hero-gradient text-primary-foreground px-4 py-2 rounded-full text-sm font-medium shadow-medium">
                      Most Popular
                    </span>
                  </div>
                )}
                <CardContent className="p-8">
                  <div className="text-center mb-8">
                    ...
                    <div className="text-4xl font-bold bg-text-gradient bg-clip-text text-transparent mb-2">{pkg.price}</div>
                  ...
                  <Button 
                    className={`w-full transition-all duration-500 hover:scale-105 transform ${
                      pkg.popular 
                        ...
                        : 'bg-secondary hover:bg-secondary/90 hover:shadow-medium'
                    }`}
                  >
                    Get Started
      ...
      <section className="py-20 relative overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-20">
          <div className="absolute top-16 left-16 w-44 h-44 bg-accent-gradient rounded-full blur-3xl animate-float"></div>
          <div className="absolute bottom-16 right-16 w-36 h-36 bg-hero-gradient rounded-full blur-3xl animate-float" style={{animationDelay: '2s'}}></div>
        </div>
        <div className="max-w-4xl mx-auto text-center px-4 sm:px-6 lg:px-8 relative">
          <h2 className="text-4xl font-bold mb-6 animate-slide-up">Ready to Transform Your Business?</h2>
